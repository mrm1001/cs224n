...
class ParserModel(Model):
    ...
    def add_placeholders(self):
        ...
        # input will be feature vectors (concatenation of word embedding indices)
        x = tf.placeholder(tf.int32, shape=(None, self.config.n_features), name='x')
        # labels will be transitions
        y = tf.placeholder(tf.float32, shape=(None, self.config.n_classes), name='y')
    ...
    def create_feed_dict(self, inputs_batch, labels_batch=None, dropout=0, beta_regul=10e-7):
        ...
        dr = self.dropout_placeholder

        if inputs_batch is not None:
            feed_dict[x] = inputs_batch
        if labels_batch is not None:
            feed_dict[y] = labels_batch
        if dropout is not None:
            feed_dict[dr] = self.config.dropout
    ...
    def add_embedding(self):
        ...
        # concatenates them
        embeddings = tf.reshape(specific_embeddings, [-1, self.config.n_features * self.config.embed_size])
        return embeddings
    ...
    def add_prediction_op(self):
        ...
        W = tf.Variable(xavier_initializer((self.config.n_features * self.config.embed_size,
                                            self.config.hidden_size)), name='W')
        self.W = W
        b1 = tf.Variable(tf.zeros((self.config.hidden_size,)), name='b')

        U = tf.Variable(xavier_initializer((self.config.hidden_size, self.config.n_classes)), name='U')
        b2 = tf.Variable(tf.zeros((self.config.n_classes,)), name='b')
