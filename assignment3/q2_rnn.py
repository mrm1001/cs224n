...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        padded_sentence = sentence[:max_length] + [zero_vector] * padding_length
        padded_label = labels[:max_length] + [zero_label] * padding_length
        mask = [True] * min([len(sentence), max_length]) + [False] * padding_length
        ret.append((padded_sentence, padded_label, mask))
...
class RNNModel(NERModel):
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE (~4-6 lines)
        embeds = tf.Variable(self.pretrained_embeddings, name='embeddings')
        # index into the embeddings
        specific_embeddings = tf.nn.embedding_lookup(embeds, self.input_placeholder)
        # concatenates them
        embeddings = tf.reshape(specific_embeddings, [-1, self.max_length, Config.n_features* Config.embed_size])
        ### END YOUR CODE
        return embeddings
    ...
    def add_prediction_op(self):
        ...
        h_0 = tf.zeros((1, Config.hidden_size))

        # input_size is (self.max_length, self.config.n_features * self.config.embed_size)
        # one cell does not care about the whole sequence, just one step
        cell = RNNCell(self.config.n_features * self.config.embed_size, self.config.hidden_size)

        with tf.variable_scope("RNN"):
            for time_step in range(self.max_length):
                x_t = x[:, time_step, :]
                # logging.info("x_t shape: {0}".format(x_t.get_shape().as_list()))
                if time_step == 0:
                    # this will create variables: W_x, W_h, b_1 (called b) in namespace called RNN
                    o_t, h_t = cell(x_t, h_0, scope="RNN")
                else:
                    tf.get_variable_scope().reuse_variables()
                    # output is (hidden_state,)
                    o_t, h_t = cell(x_t, h_t, scope="RNN")
                o_drop_t = tf.nn.dropout(o_t, self.dropout_placeholder)
                # o_drop_t will be ( x C)
                # y_t will have the shape: (C,)
                # logging.info("o_drop_t shape: {0}".format(o_drop_t.get_shape().as_list()))
                y_t = tf.matmul(o_drop_t, U) + b2
                # logging.info("y_t shape: {0}".format(y_t.get_shape().as_list()))
                preds.append(y_t)


        # Make sure to reshape @preds here.
        ### YOUR CODE HERE (~2-4 lines)
        # shape must be (batch_size or None, max_length, n_classes)
    ...
    def add_loss_op(self, preds):
        ...
        loss = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(logits=masked_logits,
