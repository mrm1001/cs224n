...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    for sentence, labels in data:
        for ind in range(len(sentence)):
            # needs padding at the beginning?
            if ind < window_size:
                start_feats = [start] * (window_size - ind) + sentence[max([0, ind - window_size]): ind]
            else:
                start_feats = sentence[max([0, ind - window_size]): ind]
            words = start_feats + sentence[ind:(ind + window_size + 1)]
            # needs padding at the end?
            if ind + window_size <= len(sentence):
                end_feats = [end] * (ind + window_size - len(sentence) + 1)
                words += end_feats

            windowed_data.append(([y for x in words for y in x], labels[ind]))
...
class WindowModel(NERModel):
    ...
    def add_placeholders(self):
        ...
        self.labels_placeholder = tf.placeholder(tf.int32, shape=(None,), name='input')
    ...
    def add_embedding(self):
        ...
        # concatenates them
        embeddings = tf.reshape(specific_embeddings, [-1, self.config.n_window_features * self.config.embed_size])
